...
class AdzunaJobScraper:
    ...
    def search_jobs(self, keywords: str, country: str, location: str = "", limit: int = 10) -> List[Dict]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not results:
                    st.warning(f"No jobs found for '{keywords}' in {country.upper()}. Try different keywords.")
                    return []
                jobs = []
                for job in results:
                    ...
                    jobs.append({
                        ...
                        'skills': self._extract_skills(description, job.get('title', '')),
                ...
                if response.status_code == 401:
                    st.error("Invalid API credentials. Check your ADZUNA_APP_ID and ADZUNA_APP_KEY.")
                return []
...
class GeminiAnalyzer:
    ...
    def extract_info(self, text: str) -> Dict:
        ...
        return {
            "skills": found[:8] if found else ['Programming'],
            "experience_years": 3,
...
with center_col:
    ...
    #job_limit = st.slider("Number of Jobs", 5, 20, 12)
    analyze_btn = st.button("🚀 Analyze Resume", use_container_width=True)

# Main analysis and output (left-aligned, no centering)
...
if analyze_btn and uploaded_file:
    ...
    with st.spinner("🔍 Analyzing your resume..."):
        try:
            ...
            jobs = scraper.search_jobs(keywords, country, location)
            if not jobs:
                st.warning("⚠️ No jobs found. Please check Adzuna credentials or try different keywords.")
                st.stop()
...
if st.session_state.analyzed and st.session_state.results:
    ...
    with tab2:
        if data['matches']:
            ...
            fig.update_layout(title="Top Job Matches by Score", xaxis_range=[0, 100],
    ...
    with stats_cols[0]:
        st.metric("Jobs Analyzed", len(data['matches']))
    with stats_cols[1]:
    ...
    st.header("🎯 Top Job Matches (Adzuna Data)")
    for job in data['matches'][:8]:
